...
export default class G6Relation extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      empty: false,
  ...
  saveData = (callBack) => {
    ...
    const {saveProjectSome, dataSource, value, projectDispatch} = this.props;
    ...
    projectDispatch.updateRelation({
...
// 离开锚点切换回编辑模式
    ...
    this.net.on('contextmenu', (ev) => {
      ...
      if (this.net._attrs.mode === 'edit') {
        ...
        if (item && item.get('type') === 'node' && item._attrs.actived) {
          contextMenus = [
            {
              name: <Space><DeleteFive theme="filled" size="12" fill={colorPrimary} strokeWidth={2} strokeLinejoin="miter"/><span>删除数据表</span></Space>,
              key: 'deleteTable',
            }];
        } else if (item && item.get('type') === 'edge' && item._attrs.actived) {
          contextMenus = [
            {
              name: <Space><RelationalGraph theme="filled" size="12" fill={colorPrimary} strokeWidth={2} strokeLinejoin="miter"/><span>对应关系</span></Space>,
              key: 'relation'
            },
            {
              name: <Space><Strikethrough theme="filled" size="12" fill={colorPrimary} strokeWidth={2} strokeLinejoin="miter"/><span>删除连接线</span></Space>,
              key: 'deleteEdge'
            },
          ];
          // 判断点击的时候是否锚点
          ...
          if (clickPoint > -1) {
            contextMenus.push(
              {
                name: <Space><CloseOne theme="filled" size="12" fill={colorPrimary} strokeWidth={2} strokeLinejoin="miter"/><span>删除锚点</span></Space>,
                key: 'deleteAnchor'
              })
          } else {
            contextMenus.push({
              name: <Space><ConnectAddressOne theme="filled" size="12" fill={colorPrimary} strokeWidth={2} strokeLinejoin="miter"/><span>添加锚点</span></Space>,
              key: 'addAnchor'
            })
          }
        } else if (!item) {
  ...
  _onDrop = (e) => {
    ...
    if (data.startsWith('map&')) {
      message.error('移动无效,该内容不是数据表，无法放置在关系图中！')
    } else {
      if (data.split('&')[1] !== moduleName) {
        message.error('移动无效,目前暂不支持跨模块之间的数据表关系配置！')
      } else {
        if (data.startsWith('entity&')) {
          ...
          let table = data.split('&')[2];
          if ((this.graphCanvas.nodes || []).some(node => node.title === table)) {
            message.error(`移动无效,关系图中已经存在表${table}`)
          } else {
